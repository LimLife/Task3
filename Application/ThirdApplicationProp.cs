...
namespace Task031023.Application
{
    public class ThirdApplicationProp : IApplicationProperty
    {
        ...
        public async Task Do()
        {
            ...
            if (employee == null) { Console.WriteLine("Data base empty or not created"); return; }
            builder.AppendLine("| № | ФИО | Дата рождения | Пол | Возраст |");
            builder.AppendLine("|---|-----|---------------|-----|---------|");
            int index = 1;
            foreach (var item in employee)
            {
                await Console.Out.WriteLineAsync(builder.AppendLine($"| {index++} | {item.FirstName} {item.LastName} {item.SurnName} | {item.DateOfBirth:dd.MM.yyyy} | {item.Gender} | {item.Age} |"));
            }
        }
    }
}
